        ...
        <AnimatePresence mode="wait">
          {showCharacter && currentCharConfig.imageUrl && preferredCharacterKey !== 'Narrator' && (
            <motion.div
              ...
              <img src={currentCharConfig.imageUrl} className="w-24 sm:w-48 rounded-3xl border border-white/10 shadow-2xl" />
            </motion.div>
          )}
        </AnimatePresence>
                  ...
                  {!isVoiceModeActive && (
                    <div className={ \`mt-12 flex items-center justify-between gap-8 \${lang === 'ar' ? 'flex-row-reverse' : ''} \` }>
                      <div className="flex items-center gap-6">
                        <button onClick={(e) => { e.stopPropagation(); handleBackScene(); }} disabled={dialogueIndex === 0} className="text-white/40 hover:text-white transition-all"><svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M15 18l-6-6 6-6"/></svg></button>
          ...
          {isEndOfScene && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center p-12 bg-black/80 rounded-3xl border border-white/10 pointer-events-auto">
              ...
              <div className="flex justify-center gap-6">
                <button onClick={handleNoChoiceAdvance} className="px-10 py-4 bg-amber-600 text-black font-bold rounded-2xl hover:bg-amber-400 transition-all">CONTINUE JOURNEY</button>
