...
type TrailerLine = {
  ...
  // Audio controls (top of component body)
    ...
    <div className="relative z-10 h-[100dvh] w-screen overflow-hidden text-white" dir={isArabic ? "rtl" : "ltr"}>
        ...
        <div className="relative z-10 flex items-center justify-between pb-3 border-b" style={{ borderColor: "rgba(255,255,255,0.10)" }}>
          ...
          <div className={`flex items-center gap-2 ${isArabic ? "flex-row-reverse" : ""}`}>
            <button
              ...
              style={{ borderColor: "rgba(255,255,255,0.16)", color: "rgba(255,255,255,0.85)" }}
            >
            ...
            <button
              onClick={() => (mediaState.isPlaying ? pause() : play())}
              className={styles.audioControl}
              style={{ borderColor: withAlpha(accent, 0.4), color: withAlpha(accent, 0.95) }}
            >
            ...
            <input
              ...
              aria-label={isArabic ? "مستوى الصوت" : "Volume"}
            />
          </div>
      ...
      <AnimatePresence>
        {mediaState.isPlaying && (
          <motion.div
            key="timer-overlay"
            className={styles.timerOverlay + " fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 pointer-events-none"}
